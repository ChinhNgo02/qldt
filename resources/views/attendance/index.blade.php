...
<link href="{{ asset('css/pages/calendar.css') }}" rel="stylesheet"
    type="text/css" />
{{-- css end --}}
@endpush @php use Illuminate\Support\Carbon; @endphp @section('content')
<div class="page-content">
    @if (session()->has('message'))
    <div class="alert alert-success">
        {{ session()->get('message') }}
    </div>
    @endif @if (auth()->user()->level == 3 || auth()->user()->level == 4 ||
    auth()->user()->level == 2)
    <section class="row">
        <div class="card">
    ...
    @endif @if (auth()->user()->level == 1) @foreach ($scheduleByClass as $key
    => $schedules)
    <section class="row">
        <div class="card">
            <div class="card-header">
