@extends('layout.master') @push('title')
<title>Classes</title>
@endpush @push('css')
{{-- css start --}}
...
<div class="page-content">
    ...
    <section class="row">
            ...
            <div class="card-body">
                <div class="col-12 d-flex justify-content-start mb-3">
                    <form class="form form-vertical" action="{{
                        Route('class.update', $class) }}" method="POST">
                        @csrf @method('PUT')
                        <div class="form-body">
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="choices form-select" id="subject" name="subject" disabled>
                                            <option value="{{ $current_subject->id
                                                }}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="weekday" name="weekday[]" class="choices form-select
                                            multiple-remove" multiple="multiple">
                                            <option value="1" @if (isset($weekdays)) @if(in_array(1, $weekdays))
                                                selected @endif @endif>
                                                Thứ 2
                                            </option>
                                            <option value="2" @if (isset($weekdays)) @if (in_array(2, $weekdays))
                                                selected @endif @endif>
                                                Thứ 3
                                            </option>
                                            <option value="3" @if (isset($weekdays)) @if (in_array(3, $weekdays))
                                                selected @endif @endif>
                                                Thứ 4
                                            </option>
                                            <option value="4" @if (isset($weekdays)) @if (in_array(4, $weekdays))
                                                selected @endif @endif>
                                                Thứ 5
                                            </option>
                                            <option value="5" @if (isset($weekdays)) @if (in_array(5, $weekdays))
                                                selected @endif @endif>
                                                Thứ 6
                                            </option>
                                            <option value="6" @if (isset($weekdays)) @if (in_array(6, $weekdays))
                                                selected @endif @endif>
                                                Thứ 7
                                            </option>
                                            <option value="7" @if (isset($weekdays)) @if (in_array(7, $weekdays))
                                                selected @endif @endif>
                                                Chủ nhật
                                            </option>
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="choices form-select" id="shift" name="shift">
                                            ...
                                            <option value="1" @if ($class->
                                                shift == 1) selected @endif>Sáng
                                            </option>
                                            <option value="2" @if ($class->
                                                shift == 2) selected
                                                @endif>Chiều
                                            </option>
                                        </select>
                                    ...
                                    <div class="form-group" id="teacherForm">
                                        ...
                                        <select id="teacher" class="choices
                                            form-select" name="teacher">
                                            <option placeholder>
                                                Chưa có giảng viên
                                            </option>
                                            @if (!empty($current_teacher))
                                            <option value="{{ $current_teacher->id
                                                }}" selected>
                                                {{ $current_teacher->name }}
                                            </option>
                                            @endif @foreach ($teachers as $data)
                                ...
                                <!-- <?php echo $class->schedule()->first()->id; ?>
                                <br> -->
                                @if (!isset($class->schedule()->first()->id))
                                <div class="col-12 d-flex justify-content-end">
                                    <button type="submit" onclick=" return
                                        validate()" class="btn btn-primary me-1
                                        mb-1">
                                        Submit
                                    </button>
                                    <button type="reset" class="btn
                                        btn-light-secondary me-1
                                        mb-1">
                                        Reset
                                    </button>
                                </div>
                                @else Lớp này đã có lịch, không thể thay đổi
                                @endif
                            </div>
                        </div>
                    </form>
        ...
        <div class="card" id="editClassStudent">
            ...
            <div class="card-body">
                <div class="col-12 d-flex justify-content-start mb-3">
                    @if($numberOfStudents >= 15) Lớp đã đủ số lượng sinh viên
                    @else
                    <button type="button" data-bs-toggle="modal" data-bs-target="#import-class-modal"
                        class="btn btn-primary me-1">
                        <i class="fas fa-file-upload"> </i> Tải lên sinh viên
                    </button>
...
@if($numberOfStudents <15) <div class="modal fade text-left modal-borderless" id="import-class-modal" tabindex="-1"
    ...
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <center>
                    <a href="{{
                        asset('files/excel/class-import-example.xlsx')
                        }}" class="btn btn-primary mb-5"><i class="fas
                            fa- download"></i>
                        Tải file mẫu</a>
                    ...
                    <div class="input-group mb-3 d-flex justify-content-center">
                        <form action="{{ route('classStudent.import', $class->id)
                            }}" method="POST" enctype="multipart/form-data" class="d-flex justify-content-start">
                            @csrf
                            ...
                            <button type="submit" class="btn btn-info" OnClick="return confirm('Are u sủe ?')">
                                Import
                            </button>
                        </form>
                    </div>
        ...
        <div class="modal-dialog modal-full modal-dialog-scrollable" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <table id="students-datatable" class="table dt-responsive nowrap
                    w-100">
                        <thead>
                            <tr>
                                <th>ID</th>
    ...
    <script>
        ...
        function checkShedule() {
            ...
            if (
                weekdays.length == 0 ||
                weekdays == "Chưa có ngày" ||
                shift == null ||
                shift.length == "Chưa có ca"
            ) {
