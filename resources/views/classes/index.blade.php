    ...
    <section class="row">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">
                    @if (auth()->user()->level >= 3 && auth()->user()->level <= 4) Quản lý lớp học @else Lớp học @endif
                        </h4>
            </div>
            <div class="card-body">
                @if (auth()->user()->level >= 3 && auth()->user()->level <= 4) <div
                    class="col-12 d-flex justify-content-start mb-3">
                    <button type="button" class="btn btn-primary me-1" data-bs-toggle="modal"
                        data-bs-target="#auto-upload-class"><i class="fas
                            fa-file-upload"> </i> Tải lên lớp
                        học</button>
                    <a href="#" type="button" class="btn btn-success me-1" data-bs-toggle="modal"
                        data-bs-target="#addSubjectModal"><i class="fas
                            fa-plus"> </i> Thêm lớp học</a>
            </div>
            ...
            @endif
            @if (auth()->user()->level >= 1 && auth()->user()->level <= 2) <table id="basic-datatable"
...
<!-- Modal -->
@if (auth()->user()->level >= 3 && auth()->user()->level <= 4) <div class="modal
    fade" id="addSubjectModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form form-vertical" action="{{ Route('class.store')
                    }}" method="POST">
                    @csrf
                    <div class="form-body">
                        <div class="row">
                            ...
                            <div class="col-12 d-flex justify-content-end">
                                <button type="submit" onclick="return
                                    validate()" class="btn btn-primary me-1 mb-1">Submit</button>
                                <button type="reset" class="btn
                                    btn-light-secondary me-1 mb-1">Reset</button>
                            </div>
                        </div>
                    </div>
                </form>
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <center>
                        <a href="{{
                        asset('files/excel/auto-upload-class-example.xlsx') }}" class="btn btn-primary mb-5"><i class="fas
                            fa-file-download"></i> Tải file mẫu</a>
    ...
    @if (auth()->user()->level >= 3 && auth()->user()->level <= 4) <script>
        ...
        if (data.status == 200) {
        return `<form action="${data.href}" method="post">
            @csrf
            @method('DELETE')
        ...
        @endif
        @if (auth()->user()->level >= 1 && auth()->user()->level <= 2) <script>
            $(document).ready(function() {
            "use strict";
            let table = $("#basic-datatable").DataTable({
            keys: !0,
